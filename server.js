...
async function startServer() {
  ...
  server.on('error', (err) => {
    if (err.code === 'EADDRINUSE') {
      console.error(`[Server] Port ${PORT} is already in use. On Windows, run: netstat -ano | findstr :${PORT}`);
      console.error('[Server] Stop the process using that PID, then run npm run dev again.');
    } else {

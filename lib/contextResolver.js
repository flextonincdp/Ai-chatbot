...
function validateSemanticResult(raw) {
  ...
  const optionalKeys = ['visualType', 'theme', 'palette', 'template', 'outputs', 'targetDocument', 'pointCount'];
  ...
  const result = {
    ...
    targetDocument: typeof raw.targetDocument === 'string' ? raw.targetDocument.trim().toLowerCase() : null,
    pointCount: raw.pointCount && !isNaN(raw.pointCount) ? parseInt(raw.pointCount, 10) : null,
...
function resolveBestContext(query, history, semanticResult = null) {
  // Guard: if the most recent assistant message is a no-evidence refusal,
  // do NOT resurrect older unrelated grounded answers for follow-up requests.
  const NO_EVIDENCE_PATTERN = /couldn't find sufficient information|i do not have enough information|does not contain information|i cannot answer|no matching chunks found/i;
  const allAssistant = (history || []).filter(m => m.role === 'assistant' && m.content);
  if (allAssistant.length > 0) {
    const lastAssistantMsg = allAssistant[allAssistant.length - 1];
    if (NO_EVIDENCE_PATTERN.test(lastAssistantMsg.content)) {
      console.log('[ContextResolver] Most recent assistant message was a no-evidence refusal — blocking context resurrection.');
      return null;
    }
  }
...
function hasChartReference(query) {
  const chartPattern = /\b(chart|graph|plot|visual|visualize|visualization)\b/i;
  const previousPattern = /\b(previous|last|that|the|current)\b/i;
  const matchesChart = chartPattern.test(query);
  const matchesPrevious = previousPattern.test(query);
  return matchesChart && (matchesPrevious || query.length < 50);
}

module.exports = { resolveContext, resolveBestContext, validateSemanticResult, hasChartReference };

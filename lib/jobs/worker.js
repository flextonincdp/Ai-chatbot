...
const { chunkStructuredText } = require('../chunk');
...
async function processJob(job) {
  ...
  try {
    ...
    const pieces = chunkStructuredText(text);
    if (!pieces.length) throw new Error('Text too short to form chunks');

    const versionId = `${docId}_v1`;
    
    // Remove existing chunks for this version to prevent duplication during reprocessing
    await query(`DELETE FROM chunks WHERE document_version_id = $1`, [versionId]);

    // Ensure chunks are inserted
    const sectionCounts = new Map();
    for (let i = 0; i < pieces.length; i++) {
      const chunkId = `${versionId}_chunk_${i}`;
      const contentHash = computeContentHash(pieces[i].text);
      const sectionKey = pieces[i].metadata.sectionNumber || pieces[i].metadata.sectionTitle || '(unsectioned)';
      sectionCounts.set(sectionKey, (sectionCounts.get(sectionKey) || 0) + 1);
      await query(`
        INSERT INTO chunks (id, document_version_id, chunk_index, content, content_hash, metadata)
        VALUES ($1, $2, $3, $4, $5, $6::jsonb)
        ON CONFLICT DO NOTHING
      `, [chunkId, versionId, i, pieces[i].text, contentHash, JSON.stringify(pieces[i].metadata)]);
    }
    console.log(`[DOCUMENT EXTRACTION] documentId=${docId} documentType=${doc.extension} sectionCount=${sectionCounts.size}`);
    for (const [section, chunkCount] of sectionCounts) console.log(`[SECTIONS] section=${section} chunkCount=${chunkCount}`);
    ...
    `);
    const vectorEnabled = vectorExtension.rows[0].enabled;

    if (provider.isConfigured() && vectorEnabled) {
      ...
      console.warn(`[Worker] pgvector is unavailable; document ${docId} will use keyword retrieval.`);
    }
